...
#define OUTPUT_FILE_RD "output_Q_PISTRO_rd.bin"
#define OUTPUT_FILE_AS "output_Q_PISTRO_as.bin"
#define OUTPUT_FILE_DE "output_Q_PISTRO_de.bin"
#define OUTPUT_FILE_FW "output_Q_PISTRO_fw.bin"
...
int main(int argc, char** arvgv) {
	...
	compute_result = Quick_Sort_PISTRO(buffer_rd, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산


	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(PISTRO) for %d elements in random order = %.3fms\n\n", n, compute_time); //random order에 대한 함수 실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(PISTRO) for %d elements in random order is falied.\n\n", n);
	...
	CHECK_TIME_START(_start, _freq); // 함수 실행 시작점 계산
	compute_result = Quick_Sort_PISTRO(buffer_as, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산

	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(PISTRO) for %d elements in ascending order = %.3fms\n\n", n, compute_time); //ascending order에 대한 함수 실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(PISTRO) for %d elements in ascending order is falied.\n\n", n);
	...
	CHECK_TIME_START(_start, _freq); // 함수 실행 시작점 계산
	compute_result = Quick_Sort_PISTRO(buffer_de, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산

	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(PISTRO) for %d elements in descending order = %.3fms\n\n", n, compute_time); //descending order에 대한  실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(PISTRO) for %d elements in descending order is falied.\n\n", n);
	...
	CHECK_TIME_START(_start, _freq); // 함수 실행 시작점 계산
	compute_result = Quick_Sort_PISTRO(buffer_fw, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산

	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(PISTRO) for %d elements in few swaps order = %.3fms\n\n", n, compute_time); //few swaps order에 대한 함수 실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(PISTRO) for %d elements in few swaps order is falied.\n\n", n);
...
int Quick_Sort_PISTRO(unsigned int* data, int left, int right) {
	...
	while (right2 - left2 > 1) {
		...
		if (pivot_index < middle) { //pivot이 왼편에 있는 경우 => 왼편이 '덩어리가 작은 쪽'
			left1 = left2;  right1 = pivot_index - 1;  //덩어리가 작은 쪽의 index인 left1, right1을 왼편 덩어리로 설정
			left2 = pivot_index + 1; right2 = right2; //덩어리가 큰 쪽의 index인 left2, right2를 오른편 덩어리로 설정
		}
		else { //pivot이 오른쪽에 있는 경우 => 오른편이 '덩어리가 작은 쪽'
			left1 = pivot_index + 1; right1 = right2; //덩어리가 작은 쪽의 index인 left1, right1을 오른편 덩어리로 설정
			left2 = left2; right2 = pivot_index - 1; //덩어리가 큰 쪽의 index인 left2, right2를 왼편 덩어리로 설정
		}
