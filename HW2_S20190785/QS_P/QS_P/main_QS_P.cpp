...
#define OUTPUT_FILE_RD "output_QS_P_rd.bin"
#define OUTPUT_FILE_AS "output_QS_P_as.bin"
#define OUTPUT_FILE_DE "output_QS_P_de.bin"
#define OUTPUT_FILE_FW "output_QS_P_fw.bin"
...
int main(int argc, char** arvgv) {
	...
	compute_result = Quick_Sort_P(buffer_rd, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산


	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(P) for %d elements in random order = %.3fms\n\n", n, compute_time); //random order에 대한 함수 실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(P) for %d elements in random order is falied.\n\n", n);
	...
	CHECK_TIME_START(_start, _freq); // 함수 실행 시작점 계산
	compute_result = Quick_Sort_P(buffer_as, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산

	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(P) for %d elements in ascending order = %.3fms\n\n", n, compute_time); //ascending order에 대한 함수 실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(P) for %d elements in ascending order is falied.\n\n", n);
	...
	CHECK_TIME_START(_start, _freq); // 함수 실행 시작점 계산
	compute_result = Quick_Sort_P(buffer_de, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산

	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(P) for %d elements in descending order = %.3fms\n\n", n, compute_time); //descending order에 대한  실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(P) for %d elements in descending order is falied.\n\n", n);
	...
	CHECK_TIME_START(_start, _freq); // 함수 실행 시작점 계산
	compute_result = Quick_Sort_P(buffer_fw, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산

	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(P) for %d elements in few swaps order = %.3fms\n\n", n, compute_time); //few swaps order에 대한 함수 실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(P) for %d elements in few swaps order is falied.\n\n", n);
...
int Quick_Sort_P(unsigned int* data, int left, int right) {
	...
	else if (size == 2) { //원소의 개수가 3개일 때, 직접 조정
