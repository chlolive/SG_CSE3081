...
int main(int argc, char** arvgv) {
	...
	compute_result = Quick_Sort_NAIVE(buffer_rd, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산


	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(NAIVE) for %d elements in random order = %.3fms\n\n", n, compute_time); //random order에 대한 함수 실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(NAIVE) for %d elements in random order is falied.\n\n", n);
	...
	CHECK_TIME_START(_start, _freq); // 함수 실행 시작점 계산
	compute_result = Quick_Sort_NAIVE(buffer_as, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산

	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(NAIVE) for %d elements in ascending order = %.3fms\n\n", n, compute_time); //ascending order에 대한 함수 실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(NAIVE) for %d elements in ascending order is falied.\n\n", n);
	...
	CHECK_TIME_START(_start, _freq); // 함수 실행 시작점 계산
	compute_result = Quick_Sort_NAIVE(buffer_de, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산

	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(NAIVE) for %d elements in descending order = %.3fms\n\n", n, compute_time); //descending order에 대한  실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(NAIVE) for %d elements in descending order is falied.\n\n", n);
	...
	CHECK_TIME_START(_start, _freq); // 함수 실행 시작점 계산
	compute_result = Quick_Sort_NAIVE(buffer_fw, 1, n); // 함수 실행
	CHECK_TIME_END(_start, _end, _freq, compute_time); // 함수 실행 종점 계산

	if (compute_result == 0) {
		fprintf(stdout, "\n^^^ Time for executing quick sort(NAIVE) for %d elements in few swaps order = %.3fms\n\n", n, compute_time); //few swaps order에 대한 함수 실행시간 출력
	}
	else fprintf(stdout, "\n^^^ The execution of quick sort(NAIVE) for %d elements in few swaps order is falied.\n\n", n);
